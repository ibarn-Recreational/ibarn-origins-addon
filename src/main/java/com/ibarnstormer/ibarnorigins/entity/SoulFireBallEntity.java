...
public class SoulFireBallEntity extends ExplosiveProjectileEntity {
    ...
    public SoulFireBallEntity(LivingEntity owner, Vec3d velocity, World world) {
    ...
    public void tick() {
        ...
        if (this.getEntityWorld().isClient() || (owner == null || !owner.isRemoved()) && this.getEntityWorld().isChunkLoaded(this.getBlockPos())) {
            ...
            this.getEntityWorld().addParticleClient(ParticleTypes.SMOKE, d, e + 0.2, f, xv, yv, zv);
        }
    }
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if(this.getEntityWorld() instanceof ServerWorld serverWorld) serverWorld.playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_DRAGON_FIREBALL_EXPLODE, SoundCategory.PLAYERS, 2.0f, 0.75f);
    }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (this.getEntityWorld() instanceof ServerWorld serverWorld) {
            ...
            if(entity2 instanceof PlayerEntity player) {
                ...
                entity.damage(serverWorld, entity.getDamageSources().indirectMagic(this, player), damage / 2);
                entity.damage(serverWorld, IODamageSources.entityDamageSource("soul_blast", player, this.getEntityWorld()), damage);
                if(entity instanceof LivingEntity livingEntity) livingEntity.addStatusEffect(new OwnableStatusEffectInstance(IOEffects.SOUL_FIRE.getRef(), 100, charged ? soulFireStrength.getAmplifier() : 0, false, false, true, player.getUuid()));
            }
            else {
                entity.damage(serverWorld, entity.getDamageSources().indirectMagic(this, entity2), 4.0F);
                entity.damage(serverWorld, IODamageSources.entityDamageSource("soul_blast", entity2, this.getEntityWorld()), 8.0F);
                if(entity instanceof LivingEntity livingEntity) livingEntity.addStatusEffect(new StatusEffectInstance(IOEffects.SOUL_FIRE.getRef(), 100, 0, false, false, true));
            }
            if (entity2 instanceof PlayerEntity player) {
                EnchantmentHelper.onTargetDamaged(serverWorld, entity, entity.getDamageSources().playerAttack(player));
            }
        }
    }
